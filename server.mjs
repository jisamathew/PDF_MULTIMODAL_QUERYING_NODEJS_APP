...
async function searchSimilarText(queryText) {
  return new Promise((resolve, reject) => {
    exec(`python3 process_text.py "embed" "${queryText.replace(/"/g, '\\"')}"`, (error, stdout, stderr) => {
      if (error) {
      ...
      if (stderr) {
        ...
        try {
          ...
          return reject(new Error(errorObj.error));
        } catch (parseError) {
...
async function answerQuestion(question, context) {
  return new Promise((resolve, reject) => {
    exec(`python3 process_text.py "qa" "${question.replace(/"/g, '\\"')}" "${context.replace(/"/g, '\\"')}"`, (error, stdout, stderr) => {
      if (error) {
        console.error('Error executing Python script:', error);
        return reject(error);
      }
      ...
      try {
        const answer = JSON.parse(stdout);  // Ensure the answer is JSON parsed
        resolve(answer);
      } catch (parseError) {
        reject(parseError);
      }
...
app.post('/search', async (req, res) => {
  ...
  try {
    ...
    const queryEmbedding = await searchSimilarText(queryText);
    // ,queryType
    const pipeline = [
      {
        "$vectorSearch": {
...
app.post('/qa', async (req, res) => {
  ...
  try {
    ...
    const questionEmbedding = await searchSimilarText(question);
    ...
    const answer = await answerQuestion(question, topResults);
